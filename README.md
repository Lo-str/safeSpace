...
A Yelp-like platform for underrepresented groups. Anyone can browse and read reviews; authenticated users can submit new spaces and reviews. React + TypeScript SPA, Express + Prisma API, PostgreSQL, Auth0 for identity.
...
Vite envs are passed as `--build-arg` because Vite bakes them at build time. The nginx config adds an SPA fallback (so React Router routes resolve on direct load), a 1-year cache on hashed assets, and `X-Content-Type-Options` / `X-Frame-Options` / `Referrer-Policy` headers.
...
[`server/src/app.ts`](server/src/app.ts) parses `CLIENT_ORIGIN` as a comma-separated allow-list and hands the array to `cors()`. No wildcards. Production sets exactly the Vercel URL (plus localhost when we want to test the local frontend against the prod backend). Anything else is rejected at the browser preflight. The [`cors.test.ts`](server/tests/integration/cors.test.ts) integration test asserts both behaviours: allowed origins are echoed in `Access-Control-Allow-Origin`, disallowed origins are not.
...
> [!TIP]
> The assignment rubric requires **"no tokens in `localStorage`"**. It does not dictate the alternative. This section explains the choice we landed on and the trade-offs.

We configure Auth0 with `cacheLocation="memory"` and `useRefreshTokens={true}`. That means:
...
Memory storage minimises blast radius: any XSS payload that runs in the SPA cannot read tokens out of a place it can inspect (`window.localStorage`, `document.cookie`) — they only exist in closures inside the Auth0 SDK. The cost is that a hard refresh or new tab triggers a redirect to Auth0 to re-establish the session. We accept that trade because (a) the redirect is fast and uses the existing Auth0 session cookie on Auth0's domain (not ours), and (b) we explicitly enabled `useRefreshTokens` and the SPA's `Allow Offline Access` to keep the in-memory session alive without forcing a redirect mid-session, which mattered for Firefox's Enhanced Tracking Protection (ETP) blocking third-party cookies used by Auth0's silent-renew iframe.

The refresh tokens themselves never touch `localStorage` — Auth0's SDK keeps them in memory alongside the access token.
...
| `deploy.yml`     | Push → main          | Re-runs both suites, then on success curls the Render + Vercel deploy hooks  |

We use **GitHub Actions as the deploy gate**: Render's and Vercel's native auto-deploy-on-push are **disabled**, so a regression that lands on `main` cannot reach production unless tests pass on the runner first. The deploy hooks are stored as repo secrets `RENDER_DEPLOY_HOOK` and `VERCEL_DEPLOY_HOOK`.

> [!NOTE]
> If you fork this repo, set both secrets and toggle off auto-deploy in Render → Service → Settings and Vercel → Project → Settings → Git for the gate to take effect.

---

## Reflection

### 1. Why did you choose this deployment platform? What were the alternatives?
...
- **Firefox-specific fix.** Firefox's Enhanced Tracking Protection blocks third-party cookies, which broke Auth0's silent-renew iframe. We switched to refresh tokens (`useRefreshTokens: true`) + enabled Allow Offline Access on the Auth0 API, so renewal no longer depends on third-party cookies.
